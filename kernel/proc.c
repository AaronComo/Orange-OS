...
extern void block(struct proc* p);
extern void unblock(struct proc *p);
 ...
 *****************************************************************************/
/**
 * <Ring 0 ~ 1> This routine is called after `p_flags' has been set (!= 0), it
 ...
 *****************************************************************************/
/**
 * <Ring 0 ~ 1> Check whether it is safe to send a message from src to dest.
...
PUBLIC void dump_msg(const char * title, MESSAGE* m)
{
  ...
  *****************************************************************************/
  /**
   * @brief <Ring 0> 检查当前用户进程是否出现栈溢出, 如果出现则提示用户
   * @author AaronComo
   *****************************************************************************/
PUBLIC void sys_canary_check() {
    ...
    if (p - proc_table >= NR_TASKS + NR_NATIVE_PROCS) {
        ...
        if (canary != 0xffffffff) {
            printl("Stack overflow occurred in process: %s\n", p->name);
            printl("Canary expected: 0xffffffff  got: 0x%x\n", canary);
        }
    }
}
